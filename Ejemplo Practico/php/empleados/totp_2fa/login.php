...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("SELECT id_empleado,usuario,password,nombre,rol,totp_secret,email FROM __TABLE__ WHERE usuario=?");
    ...
    if ($emp && $password === $emp['password']) {
        ...
        if ($emp['rol'] === 'administrador') {
            // ── Administrador: 2FA obligatorio ──────────────────────────
            if (empty($emp['email'])) {
                $error = 'Este usuario no tiene email configurado. Contacta con el administrador.';
            } else {
            ...
            // ── Empleado (u otros roles): acceso directo sin 2FA ─────────
