...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['id_empleado'])) {
        ...
        $stmt = $pdo->prepare("
            ...
            (nombre, usuario, password, especialidad, rol, email)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
...
<nav>
    <span>
        Bienvenido 
        <strong><?php echo htmlspecialchars($nombre_empleado); ?></strong>
    </span>
...
<h2>
<?php echo $edit_user ? "Editar Usuario" : "Crear Usuario"; ?>
</h2>
...
type="hidden" 
name="id_empleado" 
value="<?php echo $edit_user['id_empleado'] ?? ''; ?>"
>
...
placeholder="Usuario"
required
value="<?php echo htmlspecialchars($edit_user['usuario'] ?? ''); ?>"
>
...
value="<?php echo htmlspecialchars($edit_user['especialidad'] ?? ''); ?>"
>
...
<select name="rol" required>
    ...
    <option 
    value="profesor"
    <?php if (($edit_user['rol'] ?? '') === 'profesor') echo 'selected'; ?>
    >
        Profesor
    </option>
    ...
    <option 
    value="administrador"
    <?php if (($edit_user['rol'] ?? '') === 'administrador') echo 'selected'; ?>
    >
        Administrador
    </option>
...
<button type="submit">
<?php echo $edit_user ? "Actualizar" : "Crear Usuario"; ?>
</button>
...
<td>
<?php echo htmlspecialchars($emp['usuario']); ?>
</td>
...
<td>
<?php echo htmlspecialchars($emp['especialidad']); ?>
</td>
...
<a href="?edit=<?php echo $emp['id_empleado']; ?>">
    Editar
</a>

<a 
href="?delete=<?php echo $emp['id_empleado']; ?>"
